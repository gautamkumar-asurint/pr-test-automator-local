...
def collection_error_markers() -> tuple[str, ...]:
    """Substrings in Gradle output that signal a compile or build error.

    The fix loop uses this to bail early — Claude can't fix a compile
    error in the test file by rewriting just the test code (the issue
    might be in the imports, missing dependencies, type errors, etc.).
    """
    return _STRONG_COMPILE_ERROR_MARKERS + (
        # Also include common build-resolution failures that don't reach
        # the compile step at all
