...
== Source functions to test (these are NEW or MODIFIED) ==
...
Match the style of the existing tests above. Reuse the class-level mocks
and slots that are already declared. Do not write a package declaration,
imports, or class wrapper — those already exist.
"""
...
def user_prompt_incremental(
    ...
    ``trimmed_existing_content`` is the existing file with stale tests
    already removed. ``removed_tests_code`` is the verbatim text of those
    stale tests (a hint to Claude about prior style and coverage).
    """
    functions_code = "\n\n".join(fn.source_code for fn in affected)
