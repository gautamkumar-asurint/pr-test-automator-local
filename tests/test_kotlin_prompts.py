...
def test_handler_incremental_still_raises_stage_4b() -> None:
    """Stage 4a only — incremental merge raises NotImplementedError."""
    h = KotlinLanguageHandler()
    with pytest.raises(NotImplementedError, match="Stage 4b"):
        h.system_prompt_incremental()


def test_handler_fix_still_raises_stage_4b() -> None:
    """Stage 4a only — failure-fix prompt raises NotImplementedError."""
    h = KotlinLanguageHandler()
    with pytest.raises(NotImplementedError, match="Stage 4b"):
        h.system_prompt_fix()
