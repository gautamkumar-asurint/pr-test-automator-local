...
def test_collection_error_markers_include_compile_failures() -> None:
...
def test_collection_markers_detect_real_compile_error_fixture() -> None:
    """The compile-error fixture should contain at least one collection
    error marker — this is what tells the failure_fixer to bail rather
    than ask Claude to fix it.
    """
    markers = runner.collection_error_markers()
    assert any(marker in COMPILE_ERROR_FIXTURE for marker in markers)
...
def test_collection_markers_not_in_real_failure_fixture() -> None:
    ...
    """
    markers = runner.collection_error_markers()
    # None of the strong markers should appear in a real test-failure
    # output. (Note: "BUILD FAILED" alone is NOT in the strong markers
    # — only compile-specific ones.)
    triggered = [m for m in markers if m in FAIL_FIXTURE]
    assert triggered == [], (
        f"Unexpected compile-error markers in a test-failure scenario: "
        f"{triggered}"
    )
...
def test_handler_stage_4a_methods_now_succeed() -> None:
    """Stage 4a delivered the fresh-generation prompts. They no longer
    raise NotImplementedError (they did in v0.2.0a4).
    """
    from pr_test_automator_local.models import AffectedFunction

    h = KotlinLanguageHandler()

    # system_prompt_fresh — non-empty string
    sys_prompt = h.system_prompt_fresh()
