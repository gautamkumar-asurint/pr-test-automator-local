...
def test_handler_suggests_asurint_test_path() -> None:
    """Asurint convention strips the com/asurint/accounts/ prefix:
    tests live at unit/services/..., not unit/com/asurint/accounts/services/...
    """
...
def test_handler_suggests_for_top_level_file() -> None:
    """Non-Asurint paths keep the full path under unit/ (fallback)."""
    h = KotlinLanguageHandler()
    suggested = h.suggest_test_path(
        "src/main/kotlin/com/foo/Email.kt"
    )
    # Non-Asurint prefix → keep the com/foo/ structure
...
def test_handler_candidate_paths_include_variants() -> None:
    ...
    candidates = h.candidate_test_paths(
        "src/main/kotlin/com/asurint/accounts/services/Bar.kt"
    )
    # Default (with unit/ subdir, Asurint prefix stripped)
...
def test_unimplemented_prompts_raise_with_stage_hint() -> None:
    """Stage 4a ships fresh-generation prompts. Stage 4b (incremental
    merge and failure-fix loop) is still deferred — those methods raise.
    """
    h = KotlinLanguageHandler()

    # Stage 4a — these now succeed
    assert h.system_prompt_fresh()  # non-empty string returned

    # Stage 4b — still raise
