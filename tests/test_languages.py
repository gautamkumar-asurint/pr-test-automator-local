...
def test_unknown_extension_returns_none() -> None:
    assert get_handler_for_file("src/foo/bar.java") is None
...
def test_diff_reader_skips_non_python_extensions(tmp_path) -> None:
    """A .java file in the diff should be skipped since no Java handler
    is registered in v0.2.0a1.
    """
    ...
    assert not reader._is_eligible_source("src/Foo.java")
