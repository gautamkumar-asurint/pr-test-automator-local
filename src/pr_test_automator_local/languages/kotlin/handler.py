...
class KotlinLanguageHandler:
    ...
    def system_prompt_incremental(self) -> str:
        raise NotImplementedError(
            "Kotlin incremental merge is Stage 4b — v0.2.0a5 ships fresh "
            "generation only. Delete the existing test file to fall back "
            "to fresh generation, or wait for v0.2.0a6."
        )

    def system_prompt_fix(self) -> str:
        raise NotImplementedError(
            "Kotlin failure-fix loop is Stage 4b. v0.2.0a5 generates "
            "tests but won't attempt to fix them if Gradle reports "
            "failures. Wait for v0.2.0a6 for the fix loop."
        )
